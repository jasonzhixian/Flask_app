...
def register():
    # if login the register button is redirect to the home page
...
def login():
    # if login the login button is redirect to the home page
    if current_user.is_authenticated:
        return redirect(url_for('home'))
    form = LoginForm()
    if form.validate_on_submit():
...
#show the posts for the specific username
...
def send_reset_email(user):
    ...
    msg.body = f'''To reset your password,visit the following link：
        ...
        if you did not make this request than simply ignore this email and no change
        '''
    mail.send(msg)
